	...
	for _, qList := range j.queries {
		...
		for _, q := range qList {
			...
			if errnv != nil {
				...
				if tv, ok := q.value.(time.Time); ok {
					if tval, err := parseTime(nv); err == nil {
						nv = tval.Unix()
						q.value = tv.Unix()
					}
				}
...
func (j *JSONQ) processQuery() *JSONQ {
	...
	if aa, ok := j.jsonContent.(map[string]interface{}); ok {
		ctx := make([]interface{}, 0, len(aa))
		for _, v := range aa {
			ctx = append(ctx, v)
		}
